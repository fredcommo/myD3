...
renderJSON2 <- function(dat, fitPerf, path=NULL, fileName=NULL){
  require(RJSONIO)
  ...
  sapply(1:(nrow(fitPerf)-1), function(i){
    cat(toJSON(fitPerf[i,], container=T), ",", file = fileName, append=TRUE)
  })
  cat(toJSON(fitPerf[nrow(fitPerf),], container=T), "]", file = fileName, append=TRUE)
